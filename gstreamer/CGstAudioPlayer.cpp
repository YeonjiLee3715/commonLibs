...
void CGstAudioPlayer::setFile(const std::string &file)
{
    ...
    GstElement* source = getSource();
    g_object_set(source, "uri", file.c_str(), NULL);
}
...
GstElement *CGstAudioPlayer::getWavparse()
{
    return gst_bin_get_by_name(GST_BIN(m_parser), PARSER_WAVPARSE );
}
...
bool CGstAudioPlayer::playParser()
{
    ...
    {
        g_print("Failed to start parser. error: %d",ret);
        return false;
    }
...
bool CGstAudioPlayer::playPlayer()
{
    ...
    if(ret == GST_STATE_CHANGE_FAILURE)
    {
        g_print("Failed to start player. error: %d",ret);
        return false;
    }
...
bool CGstAudioPlayer::stopParser()
{
    ...
    if(ret == GST_STATE_CHANGE_FAILURE)
    {
        g_print("Failed to stop parser. error: %d",ret);
        return false;
    }
...
{
    if( parserState() == GST_STATE_PAUSED || parserState() == GST_STATE_NULL )
        return true;
...
{
    if( playerState() == GST_STATE_PAUSED || playerState() == GST_STATE_NULL )
        return true;
...
void CGstAudioPlayer::setParser()
{
    ...
    GstElement* wavparse = gst_element_factory_make ("wavparse", PARSER_WAVPARSE);

    // create parser pipeline
    m_parser = gst_pipeline_new (PARSER_PIPELINE);

    if (!m_parser || !appSink || !parserConvert || !source || !wavparse )
    {
    ...
    gst_bin_add_many (GST_BIN(m_parser), source, appSink, parserConvert, wavparse, NULL);
    gst_element_link_many( parserConvert, appSink, wavparse, NULL);
